...
Full rationale, decisions and the reasoning behind every constraint: **[`DESIGN.md`](./DESIGN.md)**.
...
streaming. See `DESIGN.md` §1.7 for why each one is somebody else's job.
...
Releases publish on a `v*` tag, not on merge. See `DESIGN.md` §4–5.
